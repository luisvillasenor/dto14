...
<head>
	<meta charset="utf-8">
	<title>Editar Responsables de Zona View</title>   
...
<div class="container-fluid">
  <div id="wrapper" class="row-fluid">
    <ul class="breadcrumb">
      <li><a href="<?php echo site_url();?>/resp_zona/index"><i class="icon-home"></i></a> <span class="divider">/</span></li>
      <li><a href="<?php echo site_url();?>/resp_zona/index">Responsables de Zona</a> <span class="divider">/</span></li>
      <li class="active">Actualizar Responsable de Zona</li>
    </ul>
  </div><!— /row —>
  <?php foreach ($get_rz as $key => $value) { ?>
  
  <div class="row-fluid span12">
    <!--Body content-->          
    <?php echo form_open('resp_zona/actualizar_rz'); ?>
      <fieldset>
        <legend>Actualizar Responsable de Zona ID: <?php echo $value->id_resp_zona; ?></legend>
          <input type="hidden" name="id_resp_zona" value="<?php echo $value->id_resp_zona;?>">
        ...
        <label>Nombre del Responsable de Zona</label>
          <input type="text" name="resp_zona" value="<?php echo $value->resp_zona;?>">
        <label>Sección</label>
        ...
        <label>Coordinador Asignado</label>
          <select name="id_coord">
            <?php foreach ($get_all as $item ) : ?>              
                <?php if ($value->id_coord == $item->id_coord) { ?>
                  <option value="<?php echo $item->id_coord;?>" selected="selected"><?php echo $item->coordinador;?></option>                            
                <?php }else{ ?>
