...
<head>
	<meta charset="utf-8">
	<title>Editar Jefes de Manzana View</title>   
...
<div class="container-fluid">
  <div id="wrapper" class="row-fluid">
    <ul class="breadcrumb">
      <li><a href="<?php echo site_url();?>/jefes_manzana/index"><i class="icon-home"></i></a> <span class="divider">/</span></li>
      <li><a href="<?php echo site_url();?>/jefes_manzana/index">Jefes de Manzana</a> <span class="divider">/</span></li>
      <li class="active">Actualizar Jefe de Manzana</li>
    </ul>
  </div><!— /row —>
  <?php foreach ($get_jm as $key => $value) { ?>
  
  <div class="row-fluid span12">
    <!--Body content-->          
    <?php echo form_open('jefes_manzana/actualizar_jm'); ?>
      <fieldset>
        <legend>Actualizar Jefe de Manzana ID: <?php echo $value->id_jefe_manzana; ?></legend>
          <input type="hidden" name="id_jefe_manzana" value="<?php echo $value->id_jefe_manzana;?>">
        ...
        <label>Nombre del Jefe de Manzana</label>
          <input type="text" name="jefe_manzana" value="<?php echo $value->jefe_manzana;?>">
        <label>Sección</label>
        ...
        ?>        
        <p><br>
        <label>Responsable de Sección Asignado</label>
          <select name="id_resp_seccion">
            <?php foreach ($get_all as $item ) : ?>              
                <?php if ($value->id_resp_seccion == $item->id_resp_seccion) { ?>
                  <option value="<?php echo $item->id_resp_seccion;?>" selected="selected"><?php echo $item->resp_seccion;?></option>                            
                <?php }else{ ?>
                  <option value="<?php echo $item->id_resp_seccion;?>"><?php echo $item->resp_seccion;?></option>                            
                <?php } ?>
            <?php endforeach; ?>
          </select>              
