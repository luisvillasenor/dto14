...
class Seguimiento extends CI_Controller {
	...
	public function agregar(){
		...
		$this->seguimientos_model->insert_entry();

		$res = $this->input->post('solicitud_id');

		$ultimo_id = $this->seguimientos_model->get_last_tipo($res);

		//print_r($ultimo_id);
		//$get_num['seg'] = $this->seguimientos_model->get_num_seg($res);

		//$is_tipo['t_seg'] = $this->seguimientos_model->get_tipo_seg($res);
		//foreach ($is_tipo['t_seg'] as $value) {
		//	$get_tipo = $value->tipo_seg;			
		//}

		if ($ultimo_id == '1') {

			$this->solicitudes_model->update_status_att($res);
