...
class Solicitud extends CI_Controller {
	...
	public function __construct(){ /* Esta funcion debe de ir en cada controlador para verificar si la sesion y el usuario siguen conectados */
		...
		if ( !isset($_SESSION['username'])){
			redirect(site_url()); // Redirecciona la controlador "admin"
		}
	...
	public function agregar(){
		...
		// Obtener el ID
		// Mostrar el ID en Modal Window con boton de OK que Redireccine a "captura/index"


//Ventana Modal AGREGAR SOLICITUD —>
//echo ("<div id='num_soli' class='modal hide fade in' style='display: none; '>");
//echo ("<div class='modal-header'>");    
//echo ("<h3>Número de Solicitud</h3>")      
//echo ("</div>")    
//echo ("<div class='modal-body'>")    
//echo ("<h1></h1>")   
//echo ("</div>")    
//echo ("<div class='modal-footer'>")  
//echo ("<a href='#' class='btn' data-dismiss='modal'>Cancelar &raquo;</a>");    
//echo ("</div>");
//Ventana Modal AGREGAR SOLICITUD —>
	...
	public function actualizar(){
		...
		$res = $this->input->post('solicitud_id');

		$this->solicitudes_model->update_entry($this->input->post('solicitud_id'));

		$get_num = $this->seguimientos_model->get_num_seg($res);

		$is_dep['get_dep'] = $this->solicitudes_model->get_dep($res);
		foreach ($is_dep['get_dep'] as $value) {
			$dep_id = $value->dep_id;			
		}
		...
		if ($get_num >= 1) {
			...
			if ($dep_id == 0) {
				...
				$this->solicitudes_model->update_status_pro($res);
				
			} else {

				$this->solicitudes_model->update_status_can($res);
			...
			if ($dep_id == 0) {
				...
				$this->solicitudes_model->update_status_ini($res);
...
public function buscar(){
		...
		if ($res !== '') {
			$data['get_one_solicitud'] = $this->solicitudes_model->get_one_solicitud($res);
			...
			$data['get_ciudadano_vs_solicitud'] = $this->ciudadanos_model->get_ciudadano_vs_solicitud($res);
			$this->load->view('captura_dos_view',$data);
		} else {
	...
	public function detalle(){
		...
		$data['get_one_solicitud'] = $this->solicitudes_model->get_one_solicitud($solicitud_id);
...
public function ver_seguimiento(){
		...
		if ($res !== '') {
			$data['get_one_solicitud'] = $this->solicitudes_model->get_one_solicitud($res);
			$data['get_all_referencias'] = $this->referencias_model->get_all();
			$data['get_all_seguimientos'] = $this->seguimientos_model->get_all_seguimientos($res);
        
			$data['get_ciudadano_vs_solicitud'] = $this->ciudadanos_model->get_ciudadano_vs_solicitud($res);
			$this->load->view('ver_seguimiento',$data);
		} 
	}

	public function ver_retro(){
		...
		if ($res !== '') {
			$data['get_one_solicitud'] = $this->solicitudes_model->get_one_solicitud($res);
			$data['get_all_referencias'] = $this->referencias_model->get_all();
			$data['get_all_retro'] = $this->retro_model->get_all_retro($res);
			$data['get_ciudadano_vs_solicitud'] = $this->ciudadanos_model->get_ciudadano_vs_solicitud($res);
			$this->load->view('ver_retro',$data);
		} 
	}


public function zona_graficas(){
	...
	$data['total_soli_dist'] = $this->solicitudes_model->total_soli_dist();
	$data['total_soli_sec']  = $this->solicitudes_model->total_soli_sec();
	$data['total_soli_ref']  = $this->solicitudes_model->total_soli_ref();
	
	$this->load->view('zona_graficas',$data);
}

public function zona_historicos(){
	...
	$data['get_hist_solicitudes'] = $this->solicitudes_model->get_hist_solicitudes();
	$data['get_hist_seguimientos'] = $this->seguimientos_model->get_hist_seguimientos();
	$data['get_hist_retros'] = $this->retro_model->get_hist_retros();

	$this->load->view('zona_historicos',$data);
}

public function zona_monitoreo(){
	...
	$data['get_tot_sol'] = $this->solicitudes_model->get_tot_sol();

	$data['monitoreo_status'] = $this->solicitudes_model->monitoreo_status();
	$data['moni_status_sec'] = $this->solicitudes_model->moni_status_sec();
	$data['moni_status_ref'] = $this->solicitudes_model->moni_status_ref();
	$data['moni_status_clas'] = $this->solicitudes_model->moni_status_clas();
	
	$this->load->view('zona_monitoreo',$data);
}
	...
	function notificar_administrativo(){
		...
		$solicitud_id_proveed = $this->input->post('solicitud_id_proveed');
...
Proveedor       : '.$solicitud_id_proveed.'
...
CANTIDAD        	: '.$cantidad.'
MONTO CON IVA   	: $'.number_format($monto,2).'
PROVEEDOR PROPUESTO : '.$solicitud_id_proveed.'
...
<< Este mail es enviado a través del Sistema de Solicitudes de Compra desarrollado por el área de informática >>');
	...
	function autorizar()
	{
		...
		$solicitud_id_proveed2 = $this->input->post('solicitud_id_proveed2');
...
Proveedor       : '.$solicitud_id_proveed2.'
